...
# Carregar dataset
df = pd.read_csv (r'C:\Users\jasmi\OneDrive\Área de Trabalho\RNNP_DS\PyTorch\voice.csv')
df = df.dropna()
...
train_labels = t.as_tensor(pd.Categorical(train_set.label).codes, dtype=t.long).to(device)
test_labels = t.as_tensor(pd.Categorical(test_set.label).codes, dtype=t.long).to(device)
